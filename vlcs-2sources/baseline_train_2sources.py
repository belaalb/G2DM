...
from baseline_train_loop_2sources import TrainLoop
...
parser.add_argument('--target1', type=str, default='SUN', metavar='Path', help='Path to target1 data')
...
for run in range(args.n_runs):
	...
	target_path1 = args.data_path + args.target1 + '/test/'
	target_path2 = args.data_path + args.target2 + '/test/'
	...
	test_source_dataset = Loader_unif_sampling(path1=test_source_1, path2=train_source_2, transform=img_transform_test)
	...
	target_dataset = Loader_validation(path1=target_path1, transform=img_transform_test)
...
df.to_csv('./baseline_accuracy_runs_'+args.target1+'_'+args.target2+'.csv', sep=',', index = False)
