...
parser.add_argument('--train-mode', choices = ['hv', 'avg'], default='avg', help='Train mode (options: hv, avg), (default: avg))')
...
for run in range(args.n_runs):
	...
	trainer = TrainLoop(args, models_dict, optimizer_task, source_loader, test_source_loader, target_loader, args.nadir_slack, args.alpha, args.patience, args.factor, args.smoothing, args.warmup_its, args.lr_threshold, checkpoint_path=checkpoint_path, checkpoint_epoch=args.checkpoint_epoch, cuda=args.cuda, ablation=args.ablation, logging=args.logging, train_mode=args.train_mode)
	_, results_acc, results_epochs = trainer.train(n_epochs=args.epochs, save_every=args.save_every)
...
out_file_name = args.checkpoint_path + 'out_' + args.target + '.txt'
with open(out_file_name, 'w') as out_file:
	...
	out_file.write('LR task: {}'.format(args.lr_task))
	out_file.write('LR domain: {}'.format(args.lr_domain))
	out_file.write('L2: {}'.format(args.l2))
	out_file.write('Alpha: {}'.format(args.alpha))
	out_file.write('Momentum task: {}'.format(args.momentum_task))
	out_file.write('Momentum domain: {}'.format(args.momentum_domain))
	out_file.write('Nadir slack: {}'.format(args.nadir_slack))
	out_file.write('RP size: {}'.format(args.rp_size))
	out_file.write('Patience: {}'.format(args.patience))
	out_file.write('Smoothing: {}'.format(args.smoothing))
	out_file.write('Warmup its: {}'.format(args.warmup_its))
	out_file.write('LR factor: {}'.format(args.factor))
	out_file.write('Ablation: {}'.format(args.ablation))
	out_file.write('Train mode: {}'.format(args.train_mode))
	...
	out_file.write('\nTarget acc when best source acc: {:0.4f}, epoch: {}'.format(results_acc[-3], results_epoch[-2]))
	out_file.write('\nTarget acc when best total loss: {:0.4f}, epoch: {}'.format(results_acc[2], results_epoch[2]))
	out_file.write('\nTarget acc when best task loss: {:0.4f}, epoch: {}'.format(results_acc[0], results_epoch[0]))
	out_file.write('\nTarget acc when best domain loss: {:0.4f}, epoch: {}'.format(results_acc[1], results_epoch[1]))
